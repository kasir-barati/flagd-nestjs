...
describe('Flagd Flag Evaluation E2E (PostgreSQL)', () => {
  let containers: AppContainers;

  beforeAll(async () => {
    const network = await new Network().start();
    const dbOption: DatabaseOption = await startPostgresContainer(network);
    containers = await startAppContainers(dbOption, network);
  });

  afterAll(async () => {
    await OpenFeature.close();
    await stopAppContainers('flagd-evaluation-postgres', containers);
  });
